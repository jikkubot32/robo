...
async def handle_settings_cb(e):
    if await is_admin(e.client,e.sender_id,e.chat_id):
        ...
        await e.answer("鈿狅笍 WARN 鈿狅笍 Dont Touch Admin Settings.",alert=True)
...
async def handle_upcancel_cb(e):
    ...
    if str(e.sender_id) == data[3]:
        ...
        await e.answer("Cant Cancel others upload 馃槨",alert=True)
...
async def about_me(message):
    ...
    msg = (
        ...
        "<b>Hosted By</b>: @Drake_Ry\n\n"
        "<b>Developer</b>: @yaknight\n\n"
